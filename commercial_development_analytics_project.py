...
def generate_demo_data(seed: int = 42):
    ...
    acquisition_channel = rng.choice(["Outbound", "Inbound", "Partner", "Paid"], size=n_customers, p=[0.35, 0.35, 0.15, 0.15])
    acquired_dates = pd.to_datetime("2024-01-01") + pd.to_timedelta(rng.integers(0, 730, size=n_customers), unit="D")
    ...
    order_dates = pd.to_datetime("2024-01-01") + pd.to_timedelta(rng.integers(0, 730, size=n_orders), unit="D")
    product_family = rng.choice(["Core", "Premium", "Add-on"], size=n_orders, p=[0.55, 0.25, 0.20])

    base_price = np.select(
        [product_family == "Core", product_family == "Premium", product_family == "Add-on"],
        [800, 1800, 400],
        default=700,
    )
    ...
    created_dates = pd.to_datetime("2024-01-01") + pd.to_timedelta(rng.integers(0, 700, size=n_opps), unit="D")
...
def load_data():
    ...
    if customers_path.exists() and orders_path.exists() and pipeline_path.exists():
        ...
        pipeline = pd.read_csv(pipeline_path, parse_dates=["created_date", "close_date"])
...
def calculate_kpis(orders: pd.DataFrame, pipeline: pd.DataFrame):
    ...
    gross_margin_pct = ((orders["net_revenue"].sum() - orders["cost"].sum()) / orders["net_revenue"].sum()) * 100
...
def revenue_by_segment(customers: pd.DataFrame, orders: pd.DataFrame):
    ...
    segment_df = merged.groupby("segment", as_index=False).agg(
...
def revenue_by_region(customers: pd.DataFrame, orders: pd.DataFrame):
    merged = orders.merge(customers, on="customer_id", how="left")
    region_df = merged.groupby("region", as_index=False).agg(
        revenue=("net_revenue", "sum")
    ).sort_values("revenue", ascending=False)
...
def pricing_analysis(customers: pd.DataFrame, orders: pd.DataFrame):
    ...
    pricing_df = merged.groupby("segment", as_index=False).agg(
